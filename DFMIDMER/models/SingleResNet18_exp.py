...
class Basicblock3d(nn.Module):
    ...
    def __init__(self, basicblock2d):
        ...
        self.conv1 = inflate.inflate_conv(basicblock2d.conv1, time_dim=3, time_padding=1)  # change
        self.bn1 = inflate.inflate_batch_norm(basicblock2d.bn1)
        self.conv2 = inflate.inflate_conv(basicblock2d.conv2, time_dim=3, time_padding=1)  # change
...
class SingleResNet18_exp(nn.Module):

    def __init__(self, num_classes, **kwargs):

        super(SingleResNet18_exp, self).__init__()
        resnet2d = resnet18(pretrained=True)

        self.conv1 = inflate.inflate_conv(resnet2d.conv1, time_dim=7, time_padding=3)  # change
        self.bn1 = inflate.inflate_batch_norm(resnet2d.bn1)
        self.relu = nn.ReLU(inplace=True)
        # self.relu = nn.ReLU(inplace=False)
        self.maxpool = inflate.inflate_pool(resnet2d.maxpool, time_dim=3, time_stride=1, time_padding=1)  # change
    ...
    def forward(self, x):
        ...
        # layer3
        x = self.layer3(x)
        # print('before DAO, x6', x.shape)
        _, x, masks = self.DAO(x, x)
        # print('after DAO, x6', x.shape)
...
if __name__ == '__main__':
    model = SingleResNet18_exp(num_classes=6)
