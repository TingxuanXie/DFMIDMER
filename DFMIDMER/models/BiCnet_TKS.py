...
class BiCnet_TKS(nn.Module):
    ...
    def __init__(self, num_classes, **kwargs):
        ...
        self.layer1_h2l = nn.Conv3d(256, 256 * 2, 1, stride=1, padding=0, bias=False)

        self.layer2 = self._inflate_reslayer(resnet2d.layer2)
        # self.layer2_h2l = nn.Conv3d(512, 512*3, 1, stride=1, padding=0, bias=False)
        # self.layer2_h2l = nn.Conv3d(512, 512 * 2, 1, stride=1, padding=0, bias=False)
        ...
        # self.layer3_h2l = nn.Conv3d(1024, 1024 * 2, 1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        # x_h2l = x_h2l.view(b, 2, 256, *x_h2l.size()[2:])  # [b, K, 256, 2, 32, 16]
        # x_h2l = x_h2l.permute(0, 2, 3, 1, 4, 5) #[b, 256, 2, K, 32, 16]
        # # x_h2l = x_h2l.contiguous().view(b, 256, -1, 32, 16)
        # x_h2l = x_h2l.contiguous().view(b, 256, -1, 32, 32)
        # xl = xl + x_h2l
        ...
        # x_h2l = x_h2l.view(b, 2, 512, *x_h2l.size()[2:])
        # x_h2l = x_h2l.permute(0, 2, 3, 1, 4, 5)
        # # x_h2l = x_h2l.contiguous().view(b, 512, -1, 16, 8)
        # x_h2l = x_h2l.contiguous().view(b, 512, -1, 16, 16)
        # xl = xl + x_h2l
        
        # print('before self.TKS: xh', xh.shape, 'xl', xl.shape)

        # xh, xl = self.TKS(xh, xl)
        
        # print('after self.TKS: xh', xh.shape, 'xl', xl.shape)
        # layer3
        xh, xl = self.layer3((xh, xl))
        print('after layer3 or before DAO, xh.shape=', xh.shape)
        print('after layer3 or before DAO, xl.shape=', xl.shape)
        # x_h2l = self.layer3_h2l(self.downsample(xh))
        # # x_h2l = x_h2l.view(b, 3, 1024, *x_h2l.size()[2:])
        # x_h2l = x_h2l.view(b, 2, 1024, *x_h2l.size()[2:])
        # x_h2l = x_h2l.permute(0, 2, 3, 1, 4, 5)
        # # x_h2l = x_h2l.contiguous().view(b, 1024, -1, 8, 4)
        # x_h2l = x_h2l.contiguous().view(b, 1024, -1, 8, 8)
        # xl = xl + x_h2l

        xh, xl, masks = self.DAO(xh, xl)
        print('after DAO, xh.shape=', xh.shape)
        print('after DAO, xl.shape=', xl.shape)


        # layer4
        xh, xl = self.layer4((xh, xl))
        print('after layer4 xh.shape', xh.shape)
        print('after layer4 xl.shape', xl.shape)

        xh = self.pooling(xh) #[bs, 2, c]
        xl = self.pooling(xl) #[bs, K, c]
        print('after pooling xh.shape', xh.shape)
        print('after pooling xl.shape', xl.shape)
